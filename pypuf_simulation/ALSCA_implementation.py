...
def run(k: int, n: int, N: int, seed_sim: int, noisiness: float, BATCH_SIZE: int,  weight1,  weight2) -> dict:
    ...
    epochs = 200
    print('hello')
    # challenges, responses = initialize_and_tranform_CDC_XPUF(n, k, c, N, seed_sim, noisiness, )
    ...
    outputs1 = tf.keras.layers.Dense(11, activation='softmax', name='sci')(x5)
    ...
    with open('./reliability_10xpuf_alsca_pypuf.csv', 'a') as f:

...
def run(k: int, n: int, N: int, seed_sim: int, noisiness: float, BATCH_SIZE: int,  weight1,  weight2) -> dict:
    ...
    outputs1 = tf.keras.layers.Dense(11, activation='softmax', name='new_sci')(x5)
    ...
    # breakpoint()
    fields = ['k', 'n', 'N', 'noise', 'training_size', 'test_accuracy', 'test_loss', 'time', 'seed','puf_acc','sci_weight','response_weight']

    # data rows of csv file
    rows = [[k, n, N, noisiness, len(y_train), ACC, results[0], datetime.now() - started, seed_sim, puf_acc,weight1,weight2
             ]]
    with open('./reliability_10xpuf_information_loss_pypuf.csv', 'a') as f:

...
def run(k: int, n: int, N: int, seed_sim: int, noisiness: float, BATCH_SIZE: int, weight1, weight2,num_mv) -> dict:
    ...
    # for test dataset
    test_challenges = pypuf.io.random_inputs(n=n, N=10000, seed=seed_sim + 10)
    ...
    a_1 = mv(challenges, num_mv, puf, N)
    a_1 = (a_1 + 1) / 2
    for i in range(19):
    ...
    for i in range(49):
        tmp_responses = (mv(challenges, num_mv, puf, N) + 1) / 2
        for j in range(19):
            tmp = mv(challenges, num_mv, puf, N)
            tmp = (tmp + 1) / 2
            tmp_responses += tmp
    ...
    for i in range(N):
        ...
        # tmp = np.exp(tmp) / np.sum(np.exp(tmp), axis=0)
    ...
    x5 = tf.keras.layers.Dense(64, activation=s1, kernel_initializer='random_normal')(x3)

    # x6 = tf.keras.layers.Dense(64, activation=s2, kernel_initializer='random_normal')(x3)
    x7 = tf.keras.layers.Dense(64, activation=s2, kernel_initializer='random_normal')(x3)
